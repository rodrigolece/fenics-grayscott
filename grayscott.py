...
def grayScottSolver(F_input, k_input, degree, end_time = "100.0", time_step = "1.0",
    ...
    """
    The main function to solve the Gray-Scott Equations.
    """
    ...
    w0 = interpolate(InitialConditions(cell_function = cf, sigma = sigma, degree = degree + 1), W)
    # u0, v0 = w0.split()
    # File("pvd/u0.pvd") << u0
...
# # We explored the idea of taking the "exact" solution as the solution we find
# # with a very fine mesh and a very small time step. Of course this approach is
# # wrong because the discretization introduces an error. However, we were quite
# # happy with this following class that is passed to grayScottSolver, and that
# # only allows to calculate the error if the .xml file has been saved.
# class ExactSolution():
#     def __init__(self):
#         self.saved = False
